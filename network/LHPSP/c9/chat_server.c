...
int main(int argc, char** argv)
{
	...
	struct client_data* users = (struct client_data*)malloc(
			FD_LIMIT * sizeof(struct client_data));
	assert(users != NULL);
	...
	while (1)
	{
		...
		for (i = 0; i <= user_counter; ++i)
		{
			if ((fds[i].fd == listenfd) && (fds[i].revents & POLLIN))
			{
				...
				printf("[SERVER] comes a new user, now have %d users\n", user_counter);
			}
			else if (fds[i].revents & POLLERR)
			{
				printf("[SERVER] get an error from %d\n", fds[i].fd);
				char errors[128] = {'\0'};
				socklen_t length = sizeof(errors);
				ret = getsockopt(fds[i].fd, SOL_SOCKET, SO_ERROR, &errors, &length);
				if (ret < 0)
				{
					printf("[SERVER] get socket option failed\n");
				}
				continue;
			}
			...
			else if (fds[i].revents & POLLIN)
			{
				...
				printf("[SERVER] get %d bytes of client data \"%s\" from %d\n",
